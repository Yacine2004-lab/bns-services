...
function AdminSidebar({ isOpen, onClose, onLogout, activeItem, onNavigate, adminUser }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex w-64 sm:w-72 flex-col border-r border-slate-800/80 bg-slate-950 transition-transform duration-300 ease-in-out lg:static ${
          isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
        }`}
      >
        {/* Logo */}
        <div className="flex items-center justify-between border-b border-slate-800/60 px-4 sm:px-6 py-4 sm:py-5">
        ...
        <nav className="flex-1 space-y-1 px-2 sm:px-3 py-3 sm:py-4">
          <p className="mb-2 px-2 sm:px-3 text-[10px] font-bold uppercase tracking-widest text-slate-600">Menu principal</p>
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`group relative flex w-full items-center gap-2 sm:gap-3 rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 text-xs sm:text-sm font-medium transition-all duration-200 ${
                  isActive
        ...
        <div className="border-t border-slate-800/60 p-3 sm:p-4">
          <div className="mb-3 flex items-center gap-2 sm:gap-3 rounded-xl bg-slate-900/60 px-3 sm:px-4 py-2.5 sm:py-3">
            <div className="flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-full bg-gradient-to-br from-[#e87722] to-[#d06a1a] text-[10px] sm:text-xs font-black text-[#0f2557]">
              {adminUser?.name?.[0]?.toUpperCase() || 'A'}
            </div>
            <div className="min-w-0 flex-1">
              <p className="truncate text-xs sm:text-sm font-bold text-white">{adminUser?.name || 'Administrateur'}</p>
              <p className="truncate text-[10px] sm:text-[11px] text-slate-500">{adminUser?.email || 'contact@bayeniassservices.com'}</p>
            </div>
          </div>
          <button
            type="button"
            onClick={onLogout}
            className="flex w-full items-center gap-2 sm:gap-3 rounded-xl px-3 sm:px-4 py-2 sm:py-2.5 text-xs sm:text-sm font-medium text-slate-400 transition-all duration-200 hover:bg-red-500/10 hover:text-red-400"
          >
            <LogOut size={16} />
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-2 sm:gap-3">
            <h1 className="text-xl sm:text-2xl lg:text-3xl font-black tracking-tight text-white">Tableau de bord</h1>
          </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <button
            ...
            className="inline-flex items-center gap-1.5 sm:gap-2 rounded-xl bg-[#e87722] px-4 sm:px-5 py-2 sm:py-2.5 text-xs sm:text-sm font-bold text-[#0f2557] shadow-lg shadow-[#e87722]/20 transition hover:bg-[#e87722] hover:shadow-xl"
          >
            <Package size={14} className="sm:size-16" />
            Gérer les produits
          </button>
        </div>
      </div>
      ...
      <div className="grid gap-2 sm:gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {stats.map((stat) => {
          ...
          return (
            <div
              key={stat.label}
              className="group relative overflow-hidden rounded-xl border border-slate-800/60 bg-slate-900/80 p-3 sm:p-4 transition-all duration-300 hover:border-slate-700/60 hover:shadow-lg"
            >
              <div className="flex items-start justify-between">
                ...
                <div className={`flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-lg bg-gradient-to-br ${stat.color} shadow-lg ${stat.glow}`}>
      ...
      {(lowStockProducts.length > 0 || outOfStockProducts.length > 0) && (
        <div className="rounded-2xl border border-amber-400/20 bg-amber-400/5 p-3 sm:p-5">
          <div className="mb-2 sm:mb-3 flex items-center gap-2">
            <AlertTriangle size={16} className="sm:size-18 text-amber-400" />
            <h3 className="text-xs sm:text-sm font-bold text-amber-300">Alertes stock</h3>
          </div>
          <div className="flex flex-wrap gap-2 sm:gap-3">
            {outOfStockProducts.length > 0 && (
              <div className="flex items-center gap-1.5 sm:gap-2 rounded-xl bg-red-500/10 px-3 sm:px-4 py-1.5 sm:py-2 text-[11px] sm:text-xs font-bold text-red-400 border border-red-400/20">
                <XCircle size={12} className="sm:size-14" />
                {outOfStockProducts.length} produit{outOfStockProducts.length > 1 ? 's' : ''} en rupture
              </div>
            )}
            {lowStockProducts.length > 0 && (
              <div className="flex items-center gap-1.5 sm:gap-2 rounded-xl bg-amber-500/10 px-3 sm:px-4 py-1.5 sm:py-2 text-[11px] sm:text-xs font-bold text-amber-400 border border-amber-400/20">
                <AlertTriangle size={12} className="sm:size-14" />
          ...
          <div className="mt-2 sm:mt-3 flex flex-wrap gap-1.5 sm:gap-2">
            {[...outOfStockProducts, ...lowStockProducts].slice(0, 5).map((p) => (
              <span
                key={p.id}
                className="inline-flex items-center gap-1 sm:gap-1.5 rounded-lg bg-slate-800/60 px-2.5 sm:px-3 py-1 sm:py-1.5 text-[10px] sm:text-[11px] font-medium text-slate-300"
              >
      ...
      <div className="rounded-2xl border border-slate-800/60 bg-slate-900/80">
        <div className="flex items-center justify-between border-b border-slate-800/60 px-4 sm:px-6 py-3 sm:py-4">
          <div className="flex items-center gap-2">
            <ShoppingCart size={14} className="sm:size-16 text-[#e87722]" />
            <h3 className="text-xs sm:text-sm font-bold text-white">Commandes récentes</h3>
          </div>
          <button
            type="button"
            onClick={() => onNavigate('orders')}
            className="inline-flex items-center gap-1 text-[11px] sm:text-xs font-semibold text-[#e87722] transition hover:text-[#e87722]"
          >
            Tout voir <ArrowRight size={10} className="sm:size-12" />
          </button>
        </div>

        {recentOrders.length === 0 ? (
          <div className="px-4 sm:px-6 py-8 sm:py-10 text-center">
            <ShoppingCart size={28} className="sm:size-32 mx-auto text-slate-700 mb-2 sm:mb-3" />
            <p className="text-xs sm:text-sm text-slate-500">Aucune commande pour le moment</p>
          </div>
        ) : (
          <div className="divide-y divide-slate-800/40">
            {recentOrders.map((order) => (
              <div key={order.id} className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 transition hover:bg-slate-800/20">
                <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                  <div className="flex h-8 w-8 sm:h-10 sm:w-10 flex-shrink-0 items-center justify-center rounded-xl bg-slate-800/60 text-[11px] sm:text-xs font-bold text-slate-300">
                    {order.items?.length || 0}
                  </div>
                  <div className="min-w-0">
                    <p className="text-xs sm:text-sm font-bold text-white truncate">{order.orderNumber || order.id}</p>
                ...
                <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
                  ...
                  <span className="text-xs sm:text-sm font-bold text-[#e87722]">{formatPrice(order.total)}</span>
                </div>
              </div>
            ))}
          </div>
